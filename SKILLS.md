...
A global Rust binary at `~/.local/bin/lore`. Zero dependencies, 1.7MB. Auto-detects the active project by walking up directories until it finds `lore/config.yml`. Supports UUID prefix matching on all commands.

**Install:** `install.sh` builds and places the binary. Or: `cd scripts && cargo build --release && cp target/release/lore ~/.local/bin/lore`
...
# Create from a zip (lore/ folder inside)
...
2. Run `lore session status` + `lore ticket list --status todo` — understand what's ready and what's already active
